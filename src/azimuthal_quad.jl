...
This structure holds all information related to the azimuthal quadrature, including the
discretized azimuthal angles, their associated weights for numerical integration, and the
spacing parameters used in the ray tracing algorithm.
...
This is the full number of azimuthal directions used in the discretization, spanning the
complete angular domain [0, 2π).
...
This represents the number of unique azimuthal directions needed for 2D ray tracing, since
angles in (π, 2π) are supplementary to those in (0, π).
...
This represents the number of unique directions in the first quadrant, which is used for
computing track origins and directions in the ray tracing algorithm.
...
In azimuthal quadrature, angles are divided into quadrants. The first quadrant contains
angles from 0 to π/2 radians, corresponding to indices 1 to N4.
"""
...
The second quadrant contains angles from π/2 to π radians, corresponding to indices N4+1 to
N2.
"""
...
An angle points rightward if it has a positive x-component, which corresponds to angles in
the first quadrant (indices 1 to N4).
"""
...
An angle points leftward if it has a negative x-component, which corresponds to angles in
the second quadrant (indices N4+1 to N2).
"""
...
In azimuthal quadrature, angles are organized in pairs where each angle has a supplementary
angle that differs by π radians (180°). This function computes the index of the
supplementary angle using the relationship: `supplementary_idx = N2 - i + 1`, where `N2` is
the number of azimuthal angles in the half-plane (0, π).
...
Construct an azimuthal quadrature structure for ray tracing with `N` azimuthal angles and
spacing `δ`.
...
## Returns
- `AzimuthalQuadrature{N,T,N2,N4}` where `N2 = N÷2` (half-plane angles) and `N4 = N÷4`
  (quadrant angles)
...
## Notes
The constructor initializes uninitialized vectors for effective spacings (`δs`), azimuthal
angles (`ϕs`), and weights (`ωₐ`), each of length `N2`. These arrays are populated later
...
Initialize the azimuthal quadrature weights `ωₐ` based on the computed azimuthal angles
`ϕs`.

This function computes the angular weights for numerical integration over the azimuthal
domain. The weights are calculated using a finite difference approximation of the angular
spacing between adjacent azimuthal angles.
