...
"""
    segmentize!(t::TrackGenerator; parallel=false)

Split tracks into segments generated by intersections with mesh cells/elements. Call this
after [`trace!`](@ref). Set `parallel=true` to segmentize tracks concurrently with
`Threads.@threads`; volume accumulation is still performed after all track segments are
computed.
"""
function segmentize!(
    t::TrackGenerator{T}; k::Int=5, rtol::Real=Base.rtoldefault(T), parallel::Bool=false
) where {T}
    @unpack tracks_by_uid = t
