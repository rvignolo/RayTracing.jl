...
A single `segmentize!` call only mutates each track's own segment vector while threaded work
is active; do not call `segmentize!` concurrently on the same `TrackGenerator`.
"""
function segmentize!(
